...
export function WatchFrame({ children, progress = 100 }: WatchFrameProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Render analog dial when no dynamic children are provided */}
        {!children && (
          <g opacity={screenOpacity}>
            ...
            <line
              ...
              strokeLinecap="round"
            />
            <circle cx="60" cy="74" r="2.5" fill="var(--orange, #f97316)" />
            <text
              x="60"
              y="56"
              textAnchor="middle"
            ...
            <circle cx="60" cy="100" r="2" fill="var(--orange, #f97316)" />
          </g>
        )}
      </svg>

      {/* Screen viewport overlay matching SVG screen dimensions (16/120 x 28/148) */}
      {children && (
        <div
          className="relative z-10 overflow-hidden rounded-[25px]"
          style={{
            width: `${(88 / 120) * 100}%`,
            height: `${(92 / 148) * 100}%`,
            marginTop: `${((28 - (148 - 92 - 28)) / 148) * 100}%`,
          }}
        >
          {children}
        </div>
